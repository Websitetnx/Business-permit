...
<aside class="panel decision-panel"><div class="panel-header"><div><p class="eyebrow">LGU decision</p><h3>Update application</h3></div></div><?php if ($feeAssessment): ?><div class="assessment-preview"><div><span>Calculated amount due</span><strong>₱<?= e(number_format((float) $feeAssessment['total'], 2)) ?></strong><small><?= e($feeAssessment['tax_basis_label']) ?> · ₱<?= e(number_format((float) $feeAssessment['tax_basis'], 2)) ?></small></div><details><summary>View fee breakdown</summary><dl><?php foreach ($feeAssessment['components'] as $component): ?><div><dt><?= e($component['label']) ?></dt><dd>₱<?= e(number_format((float) $component['amount'], 2)) ?></dd></div><?php endforeach; ?></dl></details></div><?php endif; ?><form method="post" class="decision-form"><?= csrf_field() ?><input type="hidden" name="application_id" value="<?= (int) $id ?>"><label class="field">Decision<select name="decision" required><?php foreach (['For Review', 'Needs Revision', 'Approved', 'Released', 'Rejected'] as $option): ?><option <?= $application['status'] === $option ? 'selected' : '' ?> <?= $option === 'Released' && (!$payment || $payment['status'] !== 'Paid') ? 'disabled' : '' ?>><?= e($option) ?></option><?php endforeach; ?></select></label><label class="field">Assessed permit fee <small><?= $feeAssessment ? 'Calculated from the saved LGU schedule' : 'Required when approving' ?></small><input name="assessed_amount" inputmode="decimal" value="<?= e($feeAssessment['total'] ?? $_POST['assessed_amount'] ?? $payment['amount'] ?? '') ?>" placeholder="0.00" <?= $feeAssessment ? 'readonly' : '' ?>></label><label class="field">BPLO notes<textarea name="admin_notes" rows="6" placeholder="Instructions or decision notes"><?= e($_POST['admin_notes'] ?? $application['admin_notes']) ?></textarea></label><?php if (!$payment || $payment['status'] !== 'Paid'): ?><p class="release-guard">Permit release is locked until payment is verified.</p><?php endif; ?><button class="button" type="submit">Save decision</button></form></aside></div>
